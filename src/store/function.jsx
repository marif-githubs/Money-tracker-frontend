// import { createContext, useEffect, useReducer, useState } from "react";
// import api from "./axios";

// export const PageData = createContext({
//     expList: [],
//     incList: [],
//     assList: [],
//     addItem: () => { },
//     deleteItem: () => { },
//     loading: "",
// })

// const dataFunction = (state, action) => {

//     let newexpList = [...state.expList];
//     let newincList = [...state.incList];
//     let newassList = [...state.assList];
//     let id = {};

//     if (action.type === "INITIAL_ITEMS") {
//         console.log(action.payload)
//         console.log(action.payload.exp_entries)
//         newexpList = action.payload.exp_entries
//         newincList = action.payload.inc_entries
//         newassList = action.payload.ass_entries

//     } else if (action.type === "ADD_ITEM") {

//         // const Controller = new AbortController();
//         const title = action.payload.title;
//         const amount = action.payload.amount;
//         const description = action.payload.description;
//         const type = action.payload.type;
//         //issue in adding item in list

//         switch (type) {
//             case "expense":
//                 api.post("http://localhost:5000/transaction/entry", {
//                     title,
//                     amount,
//                     description,
//                     type
//                 }).then(response => {
//                     console.log(response.data)
//                     id = response.data.t_id;
//                     console.log("this:", id)
//                     newexpList = [{ title, amount, description, id/*: response.data.t_id */ }, ...state.expList];
//                 })
//                     .catch(err => console.log(err));
//                 console.log({ title, amount, description, id/*: response.data.t_id */ })
//                 break;
//             case "income":
//                 api.post("http://localhost:5000/transaction/entry",
//                     {
//                         title,
//                         amount,
//                         description,
//                         type
//                     }
//                 ).then(response => {
//                     console.log(response.data)
//                     id = response.data.t_id;
//                 })
//                     .catch(err => console.log(err));
//                 newincList = [{ title, amount, description, id }, ...state.incList];
//                 break;
//             case "assets":
//                 api.post("http://localhost:5000/transaction/entry",
//                     {
//                         title,
//                         amount,
//                         description,
//                         type
//                     }
//                 ).then(response => {
//                     console.log(response.data)
//                     id = response.data.t_id;
//                 })
//                     .catch(err => console.log(err));
//                 newassList = [{ title, amount, description, id }, ...state.assList];
//                 break;
//         }

//     } else if (action.type == "DELETE_ITEM") {

//         // const Controller = new AbortController();

//         switch (action.payload.type) {
//             case "expense":
//                 newexpList = state.expList.filter(item => item.t_id !== action.payload.id);
//                 break;
//             case "income":
//                 newincList = state.incList.filter(item => item.t_id !== action.payload.id);
//                 break;
//             case "assets":
//                 newassList = state.assList.filter(item => item.t_id !== action.payload.id);
//                 break;
//         }
//         api.delete(`http://localhost:5000/transaction/entry/${action.payload.id}`)
//             .then(response => console.log(response.data))
//             .catch(err => console.log(err));

//     } else {

//         console.log("not from this");

//     }

//     return {
//         expList: newexpList,
//         incList: newincList,
//         assList: newassList
//     }
// }

// const PageDataProvider = ({ children }) => {
//     const [data, dispatch] = useReducer(dataFunction, {
//         expList: [],
//         incList: [],
//         assList: [],
//     });

//     const [loading, setLoading] = useState(false);

//     useEffect(() => {
//         setLoading(true);
//         const controller = new AbortController();
//         api.get("http://localhost:5000/transaction/entries", { signal: controller.signal })
//             .then(response => {
//                 dispatch({
//                     type: "INITIAL_ITEMS",
//                     payload: response.data,
//                 });  // Store data in state
//             })
//             .catch(error => {
//                 console.error("Error fetching data:", error);
//                 setLoading(false);
//             });
//         setLoading(false);
//         return (() => {
//             controller.abort();
//         })
//     }, []);


//     const addItem = (title, amount, description, type) => {
//         dispatch({
//             type: "ADD_ITEM",
//             payload: {
//                 title,
//                 amount,
//                 description,
//                 type
//             }

//         })

//     }

//     const deleteItem = (id, type) => {
//         dispatch({
//             type: "DELETE_ITEM",
//             payload: {
//                 id,
//                 type
//             },
//         })
//     }

//     return (
//         <PageData.Provider value={{
//             expList: data.expList,
//             incList: data.incList,
//             assList: data.assList,
//             addItem,
//             deleteItem,
//             loading
//         }}>
//             {children}
//         </PageData.Provider>
//     )
// }

// export default PageDataProvider
import { createContext, useEffect, useReducer, useState } from "react";
import api from "./axios";

export const PageData = createContext({
    expList: [],
    incList: [],
    assList: [],
    addItem: () => { },
    deleteItem: () => { },
    loading: false,
});

const dataFunction = (state, action) => {
    let newexpList = [...state.expList];
    let newincList = [...state.incList];
    let newassList = [...state.assList];
    // add initial list issue
    switch (action.type) {
        case "INITIAL_ITEMS":
            newexpList = action.payload.exp_entries
            newincList = action.payload.inc_entries
            newassList = action.payload.ass_entries
            break
        case "ADD_ITEM":
            switch (action.payload.type) {
                case "expense":
                    newexpList = [action.payload, ...state.expList]
                    break;
                case "income":
                    console.log(newincList)
                    newincList = [action.payload, ...state.incList]
                    console.log(newincList)
                    break;
                case "asset":
                    newassList = [action.payload, ...state.assList]
                    break;
            }
            break;
        case "DELETE_ITEM":
            switch (action.payload.type) {
                case "expense":
                    newexpList = state.expList.filter(item => item.t_id !== action.payload.id);
                    break;
                case "income":
                    newincList = state.incList.filter(item => item.t_id !== action.payload.id);
                    break;
                case "asset":
                    newassList = state.assList.filter(item => item.t_id !== action.payload.id);
                    break;
            }
            break;
    }
    return {
        expList: newexpList,
        incList: newincList,
        assList: newassList
    }
};

const PageDataProvider = ({ children }) => {
    const [data, dispatch] = useReducer(dataFunction, {
        expList: [],
        incList: [],
        assList: [],
    });

    const [loading, setLoading] = useState(false);

    useEffect(() => {
        setLoading(true);
        const controller = new AbortController();

        api.get("http://localhost:5000/transaction/entries", { signal: controller.signal })
            .then(response => {
                dispatch({
                    type: "INITIAL_ITEMS",
                    payload: response.data,
                });
            })
            .catch(error => {
                // console.error("Error fetching data:", error);
            })
            .finally(() => setLoading(false));

        return () => controller.abort();
    }, []);

    const addItem = async (title, amount, description, type) => {
        try {
            const response = await api.post("http://localhost:5000/transaction/entry", {
                title,
                amount,
                description,
                type
            }).then(response => {
                console.log(response.data)
                dispatch({
                    type: "ADD_ITEM",
                    payload: response.data.item
                });
            });

        } catch (error) {
            console.error("Error adding item:", error);
        }
    };

    const deleteItem = async (id, type) => {
        try {
            await api.delete(`http://localhost:5000/transaction/entry/${id}`);
            dispatch({
                type: "DELETE_ITEM",
                payload: { id, type }
            });
        } catch (error) {
            console.error("Error deleting item:", error);
        }
    };

    return (
        <PageData.Provider value={{
            expList: data.expList,
            incList: data.incList,
            assList: data.assList,
            addItem,
            deleteItem,
            loading
        }}>
            {children}
        </PageData.Provider>
    )
};

export default PageDataProvider;
